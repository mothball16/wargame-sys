return {
    LOCKDOWN_ATTR = "DS_Lockdown",

    DOOR_ROOT_TAG = "DS_DoorRoot",
    DOOR_ROOT_IDENTIFIER = "DS_Identifier",
    DOOR_ROOT_STATE_ATTR = "DS_DoorRootState",
    DOOR_ROOT_LAST_ANIMKEY_ATTR = "DS_DoorRootLastAnimKey",
    DOOR_SCANNER_PROMPT_TAG = "DS_ScannerPrompt",
    DOOR_SCANNER_SHOULD_DISPLAY_PROMPT_ATTR = "DS_ShouldDisplayPrompt",
    DOOR_SCANNER_INTERACTION_TYPE_ATTR = "DS_InteractionType",
    DOOR_SCAN_VISUAL_ATTR = "DS_ScanVisualType",

    INTERACTION_TYPE = {
        TRIGGER = "Trigger",
        PROMPT_ENTER = "PromptEnter"
    },

    ACCESS_DENIED = "Denied",
    ACCESS_ACCEPTED = "Accepted",
    ACCESS_NEUTRAL = "Neutral",
    ACCESS_DISABLED = "Disabled",

}